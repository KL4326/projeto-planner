...
const app = {
    ...
    // --- LOGS & NOTIFICAÇÕES (SISTEMA BLINDADO) ---
    async addLog(msg) {
        try {
    ...
    listenToNotifications() {
        ...
        onSnapshot(q, { includeMetadataChanges: true }, snap => {
            ...
            snap.forEach(d => {
                const dt = d.data();
                const dateObj = dt.createdAt ? new Date(dt.createdAt.seconds * 1000) : new Date();
    ...
    async openSubtaskView(sid) {
        ...
        cont.innerHTML = `
            <div class="w-full md:w-1/2 p-8 border-r dark:border-slate-800 overflow-y-auto flex flex-col gap-6 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100">
                ...
                <div><h3 class="text-3xl font-black text-primary mb-2">${d.title}</h3><p class="text-sm text-slate-500 italic whitespace-pre-line">${d.description || '...'}</p></div>
    ...
    async handleSaveProfile() { const b = document.getElementById('save-profile-btn'); const u = auth.currentUser; const f = this.tempPhotoBase64 || u.photoURL; try { await updateProfile(u, { displayName: document.getElementById('profile-name-input').value, photoURL: f }); await setDoc(doc(db,"usuarios",u.uid), { nome: document.getElementById('profile-name-input').value, cargo: document.getElementById('edit-task-desc').value, bio: document.getElementById('profile-bio-input').value, foto: f, email: u.email }, {merge:true}); this.updateAvatar(u); this.navigate('dashboard'); } catch(e) { alert("Erro ao salvar."); } },
