...
const app = {
    ...
    // --- MOTOR DE LOGS V3 (Comparação de Volume) ---
    async addLog(msg) {
    ...
    listenToNotifications() {
        onSnapshot(collection(db, "notificacoes"), snap => {
            ...
            // Lógica do Sininho (Total de Documentos no Banco vs Último Visto)
            const currentTotal = snap.size;
            if (currentTotal > this.lastLogCount) {
                badge.innerText = currentTotal - this.lastLogCount;
                badge.classList.remove('hidden');
            } else { badge.classList.add('hidden'); }

            list.innerHTML = logs.length ? '' : '<p class="p-8 text-center text-xs text-gray-400">Vazio.</p>';
            logs.slice(0, 15).forEach(dt => {
                const time = new Date(dt.ts).toLocaleTimeString('pt-PT', {hour:'2-digit', minute:'2-digit'});
                list.innerHTML += `<div class="p-4 border-b dark:border-white/5 text-left transition-colors"><p class="text-xs font-bold text-gray-700 dark:text-gray-200">${dt.text}</p><div class="flex justify-between mt-1 text-[8px] font-black uppercase text-gray-400"><span>${dt.author}</span><span>${time}</span></div></div>`;
            });
        });
    },
    ...
    renderDashboard() {
        ...
        // Stats fixas (Sempre do total independente do filtro)
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="flex flex-col gap-4 text-left">
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-3xl border dark:border-white/10 flex flex-col h-[400px] shadow-xl overflow-hidden">
                            ...
                            <div class="p-4 border-t dark:border-white/5 flex gap-2 bg-gray-50 dark:bg-black/20"><input id="chat-input" onkeydown="if(event.key === 'Enter') app.sendChatMessage()" type="text" class="flex-1 bg-white dark:bg-[#1a1c22] border-none rounded-xl px-4 text-sm outline-none dark:text-white" placeholder="Mensagem..."><button onclick="app.sendChatMessage()" class="bg-primary text-white size-10 rounded-xl flex items-center justify-center shadow-lg transition-all"><span class="material-symbols-outlined">send</span></button></div>
                        </div>
                    </div>
                </div>
    ...
    async loadProfileData() { const u = auth.currentUser; if(!u) return; const d = await getDoc(doc(db, "usuarios", u.uid)); const dt = d.data() || {}; document.getElementById('profile-name-input').value = u.displayName || ""; document.getElementById('profile-bio-input').value = dt.bio || ""; const av = document.getElementById('header-avatar'); if(u.photoURL) av.style.backgroundImage = `url('${u.photoURL}')`; else av.innerText = (u.displayName || u.email).substring(0,2).toUpperCase(); },
