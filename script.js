...
const app = {
    ...
    // --- MOTOR DE LOGS (SIMPLIFICADO E SEGURO) ---
    async addLog(msg) {
        try {
            await addDoc(collection(db, "notificacoes"), {
                text: msg,
                author: auth.currentUser.displayName || auth.currentUser.email,
                ts: Date.now() // USAR HORA DO PC PARA EVITAR ERRO DE ÍNDICE
            });
        } catch (e) { console.error("Log error:", e); }
    },

    listenToNotifications() {
        // NÃO usar orderBy no banco, ordenar manualmente no JS
        onSnapshot(collection(db, "notificacoes"), snap => {
            ...
            logs.slice(0, 15).forEach(dt => {
                ...
                list.innerHTML += `<div class="p-4 border-b dark:border-slate-800 text-left"><p class="text-xs font-bold text-slate-700 dark:text-slate-200">${dt.text}</p><div class="flex justify-between mt-2 text-[8px] font-black uppercase text-slate-400 tracking-widest"><span>${dt.author}</span><span>${time}</span></div></div>`;
            });
        });
    },
            ...
            container.innerHTML = `
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 text-left">
                    ...
                    <div class="flex flex-col gap-4">
                        <h2 class="font-black uppercase text-xs text-slate-400 p-2 text-left">Discussão</h2>
    ...
    async handleDeleteTask(id) { if(confirm("Apagar permanentemente?")) { const d = await getDoc(doc(db,"tarefas",id)); const title = d.data().title; await deleteDoc(doc(db,"tarefas",id)); await this.addLog(`🗑️ Eliminou: "${title}"`); this.navigate('dashboard'); } },
