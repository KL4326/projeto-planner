...
const app = {
    ...
    listenToNotifications() {
        onSnapshot(query(collection(db, "notificacoes"), orderBy("createdAt", "desc"), limit(20)), snap => {
            ...
            let unread = 0;
            snap.forEach(d => {
                const data = d.data(); const time = data.createdAt ? new Date(data.createdAt.seconds * 1000).toLocaleTimeString('pt-PT', {hour:'2-digit', minute:'2-digit'}) : '';
                if (data.createdAt?.seconds > this.lastReadNotif) unread++;
                list.innerHTML += `<div class="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50"><p class="text-xs font-medium">${data.text}</p><div class="flex justify-between mt-2 text-[8px] font-black text-slate-400 uppercase"><span>${data.author}</span><span>${time}</span></div></div>`;
            });
    ...
    loadFilterUsers() {} // Carregamento está integrado no loadUsers global
};
