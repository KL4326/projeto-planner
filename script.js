            ...
            container.innerHTML = `
                ...
                <div class="bg-white dark:bg-slate-900 p-8 rounded-3xl border dark:border-slate-800 shadow-xl"><h1 class="text-4xl font-black mb-4">${t.title}</h1><p class="text-slate-500 whitespace-pre-line leading-relaxed mb-8 text-sm">${t.description || 'Sem descrição.'}</p>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 border-t dark:border-slate-800 pt-6">
                        ...
                        <div class="flex flex-col gap-1 items-start"><span class="text-[9px] font-black uppercase text-slate-400 mb-2">Anexos</span><div id="task-att-list" class="flex flex-wrap gap-2"></div><button onclick="app.handleFileUpload('task', '${id}')" class="mt-2 text-[10px] font-black uppercase text-primary flex items-center gap-1"><span class="material-symbols-outlined text-sm">attach_file</span> ANEXAR</button></div>
                    </div>
                </div>
    ...
    async openSubtaskView(sid) {
        ...
        cont.innerHTML = `
            <div class="w-full md:w-1/2 p-8 border-r dark:border-slate-800 overflow-y-auto flex flex-col gap-6 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100">
                ...
                <div><h3 class="text-3xl font-black text-primary mb-2">${d.title}</h3><p class="text-sm text-slate-500 italic leading-relaxed">${d.description || 'Sem descrição.'}</p></div>
            ...
            <div class="flex-1 flex flex-col bg-slate-50 dark:bg-slate-900/40">
                ...
                <div class="p-4 border-t dark:border-slate-800 flex gap-2"><input id="sub-chat-input" type="text" class="flex-1 bg-white dark:bg-slate-800 border-none rounded-xl px-4 text-sm outline-none dark:text-white" placeholder="Chat..."><button onclick="app.sendSubComment()" class="bg-primary text-white size-10 rounded-xl flex items-center justify-center shadow-lg"><span class="material-symbols-outlined">send</span></button></div>
            </div>
        `;
