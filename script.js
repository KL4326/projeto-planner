...
const app = {
    ...
    lastReadNotif: localStorage.getItem('lastNotif') || Math.floor(Date.now() / 1000),
    ...
    bindEvents() {
        document.getElementById('login-form')?.addEventListener('submit', async (e) => { e.preventDefault(); try { await signInWithEmailAndPassword(auth, document.getElementById('login-email').value, document.getElementById('login-password').value); } catch(err) { alert("Credenciais inválidas."); } });
    ...
    checkAuth() {
        onAuthStateChanged(auth, (user) => {
            ...
            if (user) { h.classList.replace('hidden', 'flex'); this.updateAvatar(user); this.listenToTasks(); this.loadUsers(); this.navigate('dashboard'); if(user.email === "olimakl@gmail.com") document.getElementById('admin-menu-link').classList.replace('hidden', 'flex'); } 
    ...
    listenToNotifications() {
        ...
        onSnapshot(q, { includeMetadataChanges: true }, snap => {
            ...
            snap.forEach(d => {
                const dt = d.data(); const dateObj = dt.createdAt ? new Date(dt.createdAt.seconds * 1000) : new Date();
                const time = dateObj.toLocaleTimeString('pt-PT', {hour:'2-digit', minute:'2-digit'});
                if (dt.createdAt && dt.createdAt.seconds > this.lastReadNotif) unread++;
    ...
    renderDetails(id) {
        ...
        this.unsubs.push(onSnapshot(doc(db, "tarefas", id), (d) => {
            ...
            container.innerHTML = `
                ...
                <div class="bg-white dark:bg-slate-900 p-8 rounded-3xl border dark:border-slate-800 shadow-xl"><h1 class="text-4xl font-black mb-4 text-slate-900 dark:text-white">${t.title}</h1><p class="text-slate-500 dark:text-slate-400 whitespace-pre-line leading-relaxed mb-8 text-sm">${t.description || '...'}</p>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 border-t dark:border-slate-800 pt-6">
                        ...
                        <div class="flex flex-col gap-1"><span class="text-[9px] font-black uppercase text-slate-400">Setor</span><span class="text-xs font-bold">${t.sector || '---'}</span></div>
                ...
                <div class="flex gap-4 mt-6"><button id="edit-trigger" class="flex-1 bg-yellow-500 text-white py-4 rounded-2xl font-black shadow-lg uppercase text-xs tracking-widest">Editar Tarefa</button><button id="delete-btn" class="bg-red-500 text-white px-8 py-4 rounded-2xl font-black shadow-lg uppercase text-xs tracking-widest">Excluir</button></div>
            `;
            ...
            document.getElementById('edit-trigger').onclick = () => app.openEditModal(t);
    ...
    async openSubtaskView(sid) {
        ...
        cont.innerHTML = `
            <div class="w-full md:w-1/2 p-8 border-r dark:border-slate-800 overflow-y-auto flex flex-col gap-6 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100">
                ...
                <div><h3 class="text-3xl font-black text-primary mb-2">${d.title}</h3><p class="text-sm text-slate-500 dark:text-slate-400 italic leading-relaxed">${d.description || 'Sem descrição.'}</p></div>
                <div class="grid grid-cols-2 gap-4 border-t dark:border-slate-800 pt-6"><div><span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Responsáveis</span><p class="text-xs font-bold">${d.assignees?.join(', ') || '---'}</p></div><div><span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Prazo</span><p class="text-xs font-bold">${dateStr}</p></div></div>
    ...
    async handleSaveProfile() { const b = document.getElementById('save-profile-btn'); const u = auth.currentUser; const f = this.tempPhotoBase64 || u.photoURL; try { await updateProfile(u, { displayName: document.getElementById('profile-name-input').value, photoURL: f }); await setDoc(doc(db,"usuarios",u.uid), { nome: document.getElementById('profile-name-input').value, cargo: document.getElementById('profile-cargo-input').value, setor: document.getElementById('profile-sector-input').value, bio: document.getElementById('profile-bio-input').value, foto: f, email: u.email }, {merge:true}); this.updateAvatar(u); this.navigate('dashboard'); } catch(e) { alert("Erro ao salvar."); } },
