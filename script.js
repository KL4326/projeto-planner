...
const app = {
            ...
            container.innerHTML = `
                ...
                <div class="bg-white dark:bg-slate-900 p-8 rounded-3xl border dark:border-slate-800 shadow-xl">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-6 border-t dark:border-slate-800 pt-6">
                        ...
                        <div><span class="text-[9px] font-black uppercase text-slate-400">Prazo</span><p class="text-xs font-bold text-slate-700 dark:text-slate-300">${dateStr}</p></div>
    ...
    listenToChat(tid) { 
        this.unsubs.push(onSnapshot(collection(db,"tarefas",tid,"comentarios"), s => { 
            const c = document.getElementById('chat-messages'); if(!c) return;
            const msgs = s.docs.map(d=>d.data()).sort((a,b)=> (a.ts||0) - (b.ts||0));
            c.innerHTML = '';
            msgs.forEach(d => {
                const isMe = d.createdBy === auth.currentUser.uid;
                c.innerHTML += `<div class="flex flex-col ${isMe?'items-end':'items-start'}"><span class="text-[8px] font-black text-slate-400 mb-1 uppercase">${d.authorName}</span><div class="${isMe?'bg-primary text-white rounded-br-none':'bg-slate-100 dark:bg-slate-800 rounded-bl-none'} p-3 rounded-2xl text-xs shadow-sm max-w-[85%]">${d.text}</div></div>`;
            });
    ...
    async openSubtaskView(sid) {
        ...
        cont.innerHTML = `
            <div class="w-full md:w-1/2 p-8 border-r dark:border-slate-800 overflow-y-auto flex flex-col gap-6 bg-white dark:bg-slate-900">
                ...
                <div class="grid grid-cols-2 gap-4 border-t dark:border-slate-800 pt-6"><div><span class="text-[9px] font-black uppercase tracking-widest">Responsáveis</span><p class="text-xs font-bold text-slate-700 dark:text-slate-300">${d.assignees?.join(', ') || '---'}</p></div><div><span class="text-[9px] font-black uppercase tracking-widest">Prazo</span><p class="text-xs font-bold text-slate-700 dark:text-slate-300">${dateStr}</p></div></div>
    ...
    listenToSubChat(sid) { 
        this.unsubs.push(onSnapshot(query(collection(db,"tarefas",this.currentTaskId,"subtarefas",sid,"comentarios")), s => { 
            const c = document.getElementById('sub-chat-messages'); if(!c) return;
            const msgs = s.docs.map(d=>d.data()).sort((a,b)=> (a.ts||0) - (b.ts||0));
            c.innerHTML = '';
            msgs.forEach(d => {
                const isMe = d.createdBy === auth.currentUser.uid;
                c.innerHTML += `<div class="flex flex-col ${isMe?'items-end':'items-start'}"><div class="${isMe?'bg-primary text-white rounded-br-none':'bg-white dark:bg-slate-800 rounded-bl-none'} p-3 rounded-2xl text-xs shadow-sm max-w-[90%] font-medium">${d.text}</div></div>`;
            });
            c.scrollTop = c.scrollHeight;
        })); 
    },
