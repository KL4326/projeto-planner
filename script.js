...
const app = {
    ...
    listenToNotifications() {
        onSnapshot(collection(db, "notificacoes"), snap => {
            ...
            logs.slice(0, 15).forEach(dt => {
                ...
                list.innerHTML += `<div class="p-4 border-b dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors text-left"><p class="text-xs font-bold text-gray-700 dark:text-gray-200">${dt.text}</p><div class="flex justify-between mt-2 text-[8px] font-black uppercase text-gray-400"><span>${dt.author}</span><span>${time}</span></div></div>`;
            });
        });
    },
    ...
    renderDashboard() {
        ...
        filtered.forEach(t => {
            ...
            c.innerHTML += `
                <tr class="group hover:bg-gray-50 dark:hover:bg-white/5 transition-colors cursor-pointer" onclick="app.navigate('detalhes', '${t.id}')">
                    <td class="px-6 py-5 relative"><div class="status-strip bg-${s_class}"></div></td>
                    <td class="px-6 py-5"><div><p class="font-bold text-sm text-gray-900 dark:text-white ${t.status==='Concluída'?'line-through opacity-40':''}">${t.title}</p><p class="text-[11px] font-bold text-primary uppercase">Prazo: ${prazo}</p></div></td>
                    ...
                    <td class="px-6 py-5"><span class="${p.bg} text-white px-2 py-0.5 rounded text-[10px] font-black uppercase">${p.label}</span></td>
                    <td class="px-6 py-5 text-right font-bold text-xs text-gray-400">${t.assignees?.join(', ') || '---'}</td>
                </tr>`;
        });
    },
    ...
    applyStatFilter(label) { this.filters.status = label; document.querySelectorAll('.filter-btn').forEach(b => b.classList.toggle('active', b.dataset.filter === label)); this.renderDashboard(); },
    ...
    async criarTarefa() {
        ...
        await this.addLog(`➕ Criou: "${title}"`); this.navigate('dashboard');
    },
    ...
    renderDetails(id) {
        ...
        this.unsubs.push(onSnapshot(doc(db, "tarefas", id), (d) => {
            ...
            const p = CONFIG.prioridades[t.priority] || CONFIG.prioridades['Média'];
            let actionBtn = (t.status==='Em aberto') ? `<button onclick="app.updateTaskStatus('${id}', 'Em andamento')" class="bg-primary text-white px-6 py-2 rounded-xl text-xs font-black uppercase">Iniciar tarefa</button>` : (t.status==='Em andamento' ? `<button onclick="app.updateTaskStatus('${id}', 'Cancelada')" class="bg-red-500 text-white px-6 py-2 rounded-xl text-xs font-black uppercase">Cancelar tarefa</button>` : '');
            let concluirBtn = (t.status!=='Concluída'&&t.status!=='Cancelada') ? `<button onclick="app.updateTaskStatus('${id}', 'Concluída')" class="bg-emerald-500 text-white px-6 py-2 rounded-xl text-xs font-black uppercase">Concluir</button>` : '';

            container.innerHTML = `
                <div class="flex items-center justify-between"><button onclick="app.navigate('dashboard')" class="bg-white dark:bg-gray-800 p-2 rounded-xl shadow-sm border dark:border-white/10 hover:text-primary transition-all"><span class="material-symbols-outlined">arrow_back</span></button><div class="flex items-center gap-3">${actionBtn}${concluirBtn}<span class="${p.bg} text-white px-4 py-1.5 rounded-xl text-[10px] font-black uppercase shadow-lg shadow-${p.bg}/20">${p.label}</span></div></div>
                <div class="bg-white dark:bg-gray-800 p-8 rounded-3xl border border-gray-200 dark:border-white/10 shadow-xl"><h1 class="text-4xl font-black mb-4 dark:text-white">${t.title}</h1><p class="text-gray-500 dark:text-gray-400 whitespace-pre-line leading-relaxed mb-8 text-sm">${t.description || '...'}</p>
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-6 border-t dark:border-white/5 pt-6"><div><span class="text-[9px] font-black uppercase text-gray-400">Equipa</span><p class="text-xs font-bold text-primary">${t.assignees?.join(', ') || '---'}</p></div><div><span class="text-[9px] font-black uppercase text-gray-400">Prazo</span><p class="text-xs font-bold dark:text-white">${t.dueDate || '---'}</p></div><div class="flex flex-col items-start"><span class="text-[9px] font-black uppercase text-gray-400 mb-2">Anexos</span><div id="task-att-list" class="flex flex-wrap gap-2"></div><button onclick="app.handleFileUpload('task', '${id}')" class="mt-3 text-[10px] font-black uppercase text-primary flex items-center gap-1 hover:opacity-70"><span class="material-symbols-outlined text-sm">attach_file</span> ANEXAR</button></div></div>
                </div>
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="flex flex-col gap-4 text-left"><div class="flex items-center justify-between p-2 font-black text-xs text-slate-400 uppercase">Subtarefas<button onclick="app.openSubtaskForm()" class="bg-primary text-white px-4 py-2 rounded-xl text-[10px] shadow-lg">Adicionar</button></div><div id="subtasks-list" class="bg-white dark:bg-gray-800 rounded-3xl border dark:border-white/10 divide-y dark:divide-white/5 shadow-sm"></div></div>
                    <div class="flex flex-col gap-4 text-left"><h2 class="font-black uppercase text-xs text-slate-400 p-2">Discussão</h2><div class="bg-white dark:bg-gray-800 rounded-3xl border dark:border-white/10 flex flex-col h-[400px] shadow-xl overflow-hidden"><div id="chat-messages" class="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar"></div><div class="p-4 border-t dark:border-white/5 flex gap-2"><input id="chat-input" type="text" class="flex-1 bg-white dark:bg-white/5 border-none rounded-xl px-4 text-sm outline-none dark:text-white shadow-sm" placeholder="Chat..."><button onclick="app.sendChatMessage()" class="bg-primary text-white size-10 rounded-xl flex items-center justify-center shadow-lg"><span class="material-symbols-outlined">send</span></button></div></div></div>
                </div>
    ...
    async openSubtaskView(sid) {
        ...
        cont.innerHTML = `
            <div class="w-full md:w-1/2 p-8 border-r dark:border-white/10 overflow-y-auto flex flex-col gap-6 bg-white dark:bg-gray-800 text-left">
                <div class="flex items-center justify-between font-black text-[10px] uppercase text-gray-400">Subtarefa<button onclick="app.closeModal()"><span class="material-symbols-outlined">close</span></button></div>
                <div><div class="flex items-center gap-3 mb-2"><h3 class="text-3xl font-black text-primary">${d.title}</h3><span class="${p.bg} text-white px-2 py-0.5 rounded text-[8px] font-black uppercase">${p.label}</span></div><p class="text-sm text-gray-500 dark:text-gray-300 italic leading-relaxed border-l-4 border-primary/20 pl-4 py-1">${d.description || 'Sem descrição.'}</p></div>
                <div class="grid grid-cols-2 gap-4 border-t dark:border-white/5 pt-6"><div><span class="text-[9px] font-black uppercase text-gray-400">Prazo</span><p class="text-xs font-bold dark:text-white">${d.dueDate || '---'}</p></div><div class="flex flex-col items-start"><span class="text-[9px] font-black uppercase text-gray-400 mb-2">Anexos</span><div id="sub-att-list" class="flex flex-wrap gap-2"></div><button onclick="app.handleFileUpload('sub', '${sid}')" class="mt-3 text-[10px] font-black uppercase text-primary flex items-center gap-1 hover:opacity-70"><span class="material-symbols-outlined text-sm">attach_file</span> ANEXAR</button></div></div>
            ...
            <div class="flex-1 flex flex-col bg-gray-50 dark:bg-black/40 text-left">
                ...
                <div class="p-4 border-t dark:border-white/10 flex gap-2"><input id="sub-chat-input" type="text" class="flex-1 bg-white dark:bg-white/5 border-none rounded-xl px-4 text-sm outline-none dark:text-white shadow-sm" placeholder="Escreva algo..."><button onclick="app.sendSubComment()" class="bg-primary text-white size-10 rounded-xl flex items-center justify-center shadow-lg"><span class="material-symbols-outlined">send</span></button></div>
            </div>
        `;
        const sl = document.getElementById('sub-att-list'); (d.anexos || []).forEach(a => { sl.innerHTML += `<a href="${a.data}" download="${a.nome}" class="p-2 bg-white dark:bg-white/5 border dark:border-white/10 text-[9px] font-bold rounded shadow-sm">${a.name}</a>`; });
    ...
    loadUsers() { onSnapshot(collection(db,"usuarios"), snap => { const opts = snap.docs.map(d => d.data().nome); ['task-assignees-checkboxes', 'edit-assignees-checkboxes', 'sub-assignees-checkboxes'].forEach(cid => { const el = document.getElementById(cid); if(el) el.innerHTML = opts.map(n => `<label class="flex items-center gap-2 p-2 hover:bg-gray-100 dark:hover:bg-white/5 rounded cursor-pointer"><input type="checkbox" value="${n}" class="${cid}-item rounded text-primary w-4 h-4"><span class="text-xs font-bold">${n}</span></label>`).join(''); }); }); },
    ...
    listenToSubtasks(tid) { this.unsubs.push(onSnapshot(collection(db,"tarefas",tid,"subtarefas"), s => { const l = document.getElementById('subtasks-list'); if(l) { const sts = s.docs.map(d=>({id:d.id, ...d.data()})).sort((a,b)=> (a.ts_manual||0) - (b.ts_manual||0)); l.innerHTML = sts.length ? sts.map(st => `<div class="flex items-center gap-3 px-6 py-4 hover:bg-gray-50 dark:hover:bg-white/5 cursor-pointer text-left" onclick="if(event.target.type !== 'checkbox') app.openSubtaskView('${st.id}')"><input type="checkbox" ${st.completed?'checked':''} onchange="app.toggleSub('${st.id}', this.checked)" class="rounded text-primary w-5 h-5"><span class="flex-1 text-sm font-bold ${st.completed?'subtask-done text-gray-400':'dark:text-white'}">${st.title}</span><span class="material-symbols-outlined text-gray-300 text-sm">chevron_right</span></div>`).join('') : '<p class="p-8 text-center text-xs text-gray-400">Nenhuma etapa.</p>'; } })); },
