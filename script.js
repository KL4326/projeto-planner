...
const app = {
    ...
    listenToNotifications() {
        onSnapshot(collection(db, "notificacoes"), snap => {
            ...
            list.innerHTML = logs.length ? '' : '<p class="p-8 text-center text-xs text-gray-400 italic">Sem logs.</p>';
    ...
    markNotifsRead() { this.lastLogCount = this.lastLogCount + (parseInt(document.getElementById('notif-badge').innerText) || 0); localStorage.setItem('lastLogCount', this.lastLogCount); document.getElementById('notif-badge').classList.add('hidden'); },

    // --- TAREFAS (ISOLADAS DE ÍNDICES) ---
    listenToTasks() { onSnapshot(collection(db, "tarefas"), snap => { this.allTasks = snap.docs.map(d => ({id: d.id, ...d.data()})); this.renderDashboard(); this.renderRanking(); }); },
    renderDashboard() {
    ...
    async criarTarefa() {
        ...
        await this.addLog(`➕ Criou a tarefa: "${title}"`); this.navigate('dashboard');
    },
    ...
    renderDetails(id) {
        ...
        this.unsubs.push(onSnapshot(doc(db, "tarefas", id), (d) => {
            if(!d.exists()) return;
            const t = d.data(); const p = CONFIG.prioridades[t.priority] || CONFIG.prioridades['Média'];
            let btns = (t.status==='Em aberto' || t.status==='Atrasado') ? `<button onclick="app.updateTaskStatus('${id}', 'Em andamento')" class="bg-primary text-white px-6 py-2 rounded-xl text-xs font-black uppercase">Iniciar</button><button onclick="app.updateTaskStatus('${id}', 'Concluída')" class="bg-emerald-500 text-white px-6 py-2 rounded-xl text-xs font-black uppercase">Concluir</button>` : (t.status==='Em andamento' ? `<button onclick="app.updateTaskStatus('${id}', 'Cancelada')" class="bg-red-500 text-white px-6 py-2 rounded-xl text-xs font-black uppercase">Cancelar</button><button onclick="app.updateTaskStatus('${id}', 'Concluída')" class="bg-emerald-500 text-white px-6 py-2 rounded-xl text-xs font-black uppercase">Concluir</button>` : `<button onclick="app.updateTaskStatus('${id}', 'Em aberto')" class="bg-blue-500 text-white px-6 py-2 rounded-xl text-xs font-black uppercase flex items-center gap-1"><span class="material-symbols-outlined text-sm">refresh</span> Reabrir</button>`);

            container.innerHTML = `
                <div class="flex items-center justify-between"><button onclick="app.navigate('dashboard')" class="bg-white dark:bg-gray-800 p-2 rounded-xl shadow-sm border dark:border-white/10 hover:text-primary transition-all"><span class="material-symbols-outlined">arrow_back</span></button><div class="flex items-center gap-3">${btns}<span class="${p.bg} text-white px-4 py-1.5 rounded-xl text-[10px] font-black uppercase shadow-lg">${p.label}</span></div></div>
                <div class="bg-white dark:bg-gray-800 p-8 rounded-3xl border border-gray-200 dark:border-white/10 shadow-xl text-left"><h1 class="text-4xl font-black mb-4 dark:text-white">${t.title}</h1><p class="text-gray-500 dark:text-gray-400 whitespace-pre-line leading-relaxed mb-8 text-sm">${t.description || '...'}</p>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="flex flex-col gap-4 text-left"><h2 class="font-black uppercase text-xs text-slate-400 p-2">Discussão</h2><div class="bg-white dark:bg-gray-800 rounded-3xl border dark:border-white/10 flex flex-col h-[400px] shadow-xl overflow-hidden"><div id="chat-messages" class="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar"></div><div class="p-4 border-t dark:border-white/5 flex gap-2 bg-gray-50 dark:bg-black/20"><input id="chat-input" onkeydown="if(event.key==='Enter')app.sendChatMessage()" type="text" class="flex-1 bg-white dark:bg-[#1a1c22] border-none rounded-xl px-4 text-sm outline-none dark:text-white" placeholder="Escreva..."><button onclick="app.sendChatMessage()" class="bg-primary text-white size-10 rounded-xl flex items-center justify-center shadow-lg"><span class="material-symbols-outlined">send</span></button></div></div></div>
                </div>
                <div class="flex gap-4 mt-6"><button onclick="app.openEditModal()" class="flex-1 bg-yellow-500 text-white py-4 rounded-2xl font-black shadow-lg uppercase text-xs tracking-widest hover:brightness-110">Editar Tarefa</button><button onclick="app.handleDeleteTask('${id}')" class="bg-red-500 text-white px-8 py-4 rounded-2xl font-black shadow-lg uppercase text-xs tracking-widest hover:brightness-110">Excluir</button></div>
            `;
    ...
    async updateTaskStatus(id, newStatus) { await updateDoc(doc(db, "tarefas", id), { status: newStatus }); await this.addLog(`🔄 Mudou para "${newStatus}"`); },
    ...
    async openSubtaskView(sid) {
        ...
        cont.innerHTML = `
            <div class="w-full md:w-1/2 p-8 border-r dark:border-white/10 overflow-y-auto flex flex-col gap-6 bg-white dark:bg-[#1a1c22] text-left">
                ...
                <div class="grid grid-cols-2 gap-4 border-t dark:border-white/5 pt-6"><div><span class="text-[9px] font-black uppercase text-gray-400">Responsáveis</span><p class="text-xs font-bold dark:text-white">${d.assignees?.join(', ') || '---'}</p></div><div><span class="text-[9px] font-black uppercase text-gray-400">Prazo</span><p class="text-xs font-bold dark:text-white">${d.dueDate || '---'}</p></div></div>
        ...
        document.getElementById('modal-backdrop').classList.add('flex'); document.getElementById('modal-backdrop').classList.remove('hidden'); document.getElementById('modal-subtask-view').classList.remove('hidden');
    ...
    listenToSubtasks(tid) { this.unsubs.push(onSnapshot(collection(db,"tarefas",tid,"subtarefas"), s => { const l = document.getElementById('subtasks-list'); if(l) { const sts = s.docs.map(d=>({id:d.id, ...d.data()})).sort((a,b)=> (a.ts_manual||0) - (b.ts_manual||0)); l.innerHTML = sts.length ? sts.map(st => { const p = CONFIG.prioridades[st.priority] || CONFIG.prioridades['Média']; return `<div class="flex items-center gap-3 px-6 py-4 hover:bg-gray-50 dark:hover:bg-white/5 cursor-pointer text-left" onclick="if(event.target.type !== 'checkbox') app.openSubtaskView('${st.id}')"><input type="checkbox" ${st.completed?'checked':''} onchange="app.toggleSub('${st.id}', this.checked)" class="rounded text-primary w-5 h-5"><div class="flex-1 flex items-center justify-between gap-2"><span class="text-sm font-bold ${st.completed?'subtask-done text-gray-400':'dark:text-white'}">${st.title}</span><span class="${p.bg} text-white px-2 py-0.5 rounded text-[8px] font-black uppercase opacity-90">${p.label}</span></div><span class="material-symbols-outlined text-gray-300 text-sm">chevron_right</span></div>`; }).join('') : '<p class="p-8 text-center text-xs text-gray-400 italic">Nenhuma etapa.</p>'; } })); },
    ...
    async openEditModal() { const d = await getDoc(doc(db,"tarefas",this.currentTaskId)); const t = d.data(); document.getElementById('edit-task-title').value = t.title; document.getElementById('edit-task-desc').value = t.description || ""; document.getElementById('edit-task-priority').value = t.priority || "Média"; document.getElementById('edit-task-date').value = t.dueDate || ""; document.querySelectorAll('.edit-assignees-checkboxes-item').forEach(cb => cb.checked = t.assignees?.includes(cb.value)); document.getElementById('modal-backdrop').classList.add('flex'); document.getElementById('modal-backdrop').classList.remove('hidden'); document.getElementById('modal-edit-task').classList.remove('hidden'); },
    ...
    deleteSub(sid) { if(confirm("Remover?")) { deleteDoc(doc(db,"tarefas",this.currentTaskId,"subtarefas",sid)); this.addLog("🗑️ Removeu uma subtarefa"); this.closeModal(); } }
};
