...
const app = {
    ...
    init() { 
        app.currentReminderDate = app.getTodayStr();
        app.bindEvents(); 
        app.checkAuth(); 
        app.initTheme(); 
    },
    ...
    renderDetails(id) {
        ...
        app.unsubs.push(onSnapshot(doc(db, "tarefas", id), (d) => {
            ...
            (t.assignees || []).forEach(name => {
                const uData = Object.values(app.userMap).find(u => u.nome === name);
                if(uData && uData.foto) avatarsHtml += `<div class="w-10 h-10 rounded-full border-2 border-white dark:border-[#151c2c] bg-cover bg-center shadow-sm" style="background-image:url('${uData.foto}')" title="${name}"></div>`;
                else avatarsHtml += `<div class="w-10 h-10 rounded-full border-2 border-white dark:border-[#151c2c] bg-primary text-white flex items-center justify-center font-bold text-[11px] shadow-sm" title="${name}">${name.substring(0,2).toUpperCase()}</div>`;
            });
            ...
            container.innerHTML = `
                ...
                <div class="glass-panel p-8 rounded-3xl border dark:border-white/5 shadow-xl flex flex-col md:flex-row justify-between gap-8 bento-highlight dark:bg-[#151c2c]">
                    ...
                    <div class="md:w-48 border-t md:border-t-0 md:border-l border-gray-200 dark:border-white/5 pt-6 md:pt-0 md:pl-8 text-left flex flex-col">
                        ...
                        <div class="flex flex-wrap gap-1">${avatarsHtml || '<span class="text-xs font-bold text-gray-400 uppercase">Não definido</span>'}</div>
                    </div>
                </div>
    ...
    async handleDeleteTask(id) { if(confirm("Excluir tarefa?")) { const d = await getDoc(doc(db,"tarefas",id)); const title = d.exists() ? d.data().title : 'Tarefa'; await deleteDoc(doc(db,"tarefas",id)); app.addLog(`🗑️ Excluiu a tarefa: "${title}"`); app.navigate('dashboard'); } },
    ...
    async handleSaveProfile() { try { await updateProfile(auth.currentUser, { displayName: document.getElementById('profile-name-input').value }); const novaFoto = app.tempPhotoBase64; const updateObj = { nome: document.getElementById('profile-name-input').value, cargo: document.getElementById('profile-role-input').value, bio: document.getElementById('profile-bio-input').value }; if (novaFoto !== null) updateObj.foto = novaFoto; await setDoc(doc(db,"usuarios",auth.currentUser.uid), updateObj, {merge:true}); document.getElementById('user-display-name').innerText = document.getElementById('profile-name-input').value; document.getElementById('user-display-role').innerText = document.getElementById('profile-role-input').value; const avH = document.getElementById('header-avatar'); if (novaFoto) { avH.style.backgroundImage = `url('${novaFoto}')`; avH.innerText = ''; } else if (novaFoto === "") { avH.style.backgroundImage = 'none'; avH.innerText = auth.currentUser.displayName.substring(0,2).toUpperCase(); } app.showToast("Perfil corporativo updated!"); app.navigate('dashboard'); } catch(e) { app.showToast("Erro ao salvar", "error"); } },
