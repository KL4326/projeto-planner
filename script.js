...
const app = {
    ...
    bindEvents() {
        document.getElementById('login-form')?.addEventListener('submit', async (e) => { e.preventDefault(); try { await signInWithEmailAndPassword(auth, document.getElementById('login-email').value, document.getElementById('login-password').value); } catch(err) { alert("Dados incorretos."); } });
    ...
    // --- LOGS & NOTIFICAÇÕES (RECONSTRUÍDO) ---
    async addLog(msg) {
        try {
            await addDoc(collection(db, "notificacoes"), {
                ...
                backupTime: Date.now() // Backup caso o serverTimestamp demore
            });
    ...
    listenToNotifications() {
        ...
        onSnapshot(q, { includeMetadataChanges: true }, snap => {
            ...
            snap.forEach(d => {
                const dt = d.data();
                const ts = dt.createdAt ? dt.createdAt.seconds : (dt.backupTime / 1000);
    ...
    deleteSub(sid) { if(confirm("Eliminar?")) { deleteDoc(doc(db,"tarefas",this.currentTaskId,"subtarefas",sid)); this.closeModal(); } },
