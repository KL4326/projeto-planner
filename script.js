...
const app = {
    ...
    listenToNotifications() {
        onSnapshot(collection(db, "notificacoes"), snap => {
            ...
            logs.slice(0, 15).forEach(dt => {
                ...
                list.innerHTML += `<div class="p-4 border-b dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors text-left"><p class="text-xs font-bold text-gray-700 dark:text-gray-200">${dt.text}</p><div class="flex justify-between mt-2 text-[8px] font-black uppercase text-gray-400 tracking-widest"><span>${dt.author}</span><span>${time}</span></div></div>`;
            });
        });
    },
        ...
        filtered.forEach(t => {
            ...
            c.innerHTML += `
                <tr class="group hover:bg-gray-50 dark:hover:bg-white/5 transition-colors cursor-pointer" onclick="app.navigate('detalhes', '${t.id}')">
                    <td class="px-6 py-5 relative"><div class="status-strip bg-${s_slug}"></div></td>
                    ...
                    <td class="px-6 py-5"><span class="px-2.5 py-1 rounded-full text-[10px] font-black uppercase bg-${s_slug}/10 text-${s_slug}">${t.status}</span></td>
                    <td class="px-6 py-5"><span class="${p.bg} text-white px-2.5 py-0.5 rounded text-[10px] font-black uppercase">${p.label}</span></td>
    ...
    async criarTarefa() {
        ...
        await this.addLog(`➕ Adicionou: "${title}"`); this.navigate('dashboard');
    },
    ...
    renderDetails(id) {
        ...
        this.unsubs.push(onSnapshot(doc(db, "tarefas", id), (d) => {
            ...
            const p = CONFIG.prioridades[t.priority] || CONFIG.prioridades['Média'];
            let actionBtn = (t.status==='Em aberto') ? `<button onclick="app.updateTaskStatus('${id}', 'Em andamento')" class="bg-primary text-white px-6 py-2 rounded-xl text-xs font-black uppercase shadow-lg shadow-primary/20">Iniciar tarefa</button>` : (t.status==='Em andamento' ? `<button onclick="app.updateTaskStatus('${id}', 'Cancelada')" class="bg-red-500 text-white px-6 py-2 rounded-xl text-xs font-black uppercase shadow-lg shadow-red-500/20">Cancelar tarefa</button>` : '');
            let concluirBtn = (t.status!=='Concluída'&&t.status!=='Cancelada') ? `<button onclick="app.updateTaskStatus('${id}', 'Concluída')" class="bg-emerald-500 text-white px-6 py-2 rounded-xl text-xs font-black uppercase shadow-lg shadow-emerald-500/20">Concluir</button>` : '';
            ...
            container.innerHTML = `
                ...
                <div class="bg-white dark:bg-gray-800 p-8 rounded-3xl border border-gray-200 dark:border-white/10 shadow-xl"><h1 class="text-4xl font-black mb-4 dark:text-white">${t.title}</h1><p class="text-gray-500 dark:text-gray-400 whitespace-pre-line leading-relaxed mb-8 text-sm">${t.description || 'Sem descrição.'}</p>
            ...
            `;
            const al = document.getElementById('task-att-list'); (t.anexos || []).forEach(a => { al.innerHTML += `<a href="${a.data}" download="${a.nome}" class="p-2 bg-gray-100 dark:bg-white/5 rounded-lg border dark:border-white/10 text-[9px] font-bold truncate max-w-[140px] hover:text-primary transition-all">${a.name}</a>`; });
    ...
    async openSubtaskView(sid) {
        ...
        cont.innerHTML = `
            <div class="w-full md:w-1/2 p-8 border-r dark:border-white/10 overflow-y-auto flex flex-col gap-6 bg-white dark:bg-[#1a1c22] text-left">
                ...
                <div class="grid grid-cols-2 gap-4 border-t dark:border-white/5 pt-6"><div><span class="text-[9px] font-black uppercase text-gray-400">Responsáveis</span><p class="text-xs font-bold text-gray-700 dark:text-gray-200">${d.assignees?.join(', ') || '---'}</p></div><div><span class="text-[9px] font-black uppercase text-gray-400">Prazo</span><p class="text-xs font-bold dark:text-white">${d.dueDate ? new Date(d.dueDate).toLocaleDateString('pt-PT') : '---'}</p></div></div>
                <div class="flex flex-col border-t dark:border-white/5 pt-4"><div id="sub-att-list" class="flex flex-wrap gap-2"></div><button onclick="app.handleFileUpload('sub', '${sid}')" class="mt-3 text-[10px] font-black uppercase text-primary flex items-center gap-1 hover:opacity-70 transition-all"><span class="material-symbols-outlined text-sm">attach_file</span> ANEXAR</button></div>
    ...
    async handleSaveSubtask() { const t = document.getElementById('sub-title-inp').value; if(!t) return; const assignees = Array.from(document.querySelectorAll('.sub-assignees-checkboxes-item:checked')).map(cb => cb.value); const data = { title: t, description: document.getElementById('sub-desc-inp').value, priority: document.getElementById('sub-priority-inp').value, dueDate: document.getElementById('sub-date-inp').value, assignees, ts_manual: Date.now() }; if (this.editSubId) await updateDoc(doc(db, "tarefas", this.currentTaskId, "subtarefas", this.editSubId), data); else await addDoc(collection(db, "tarefas", this.currentTaskId, "subtarefas"), { ...data, completed: false, createdAt: serverTimestamp() }); this.closeModal(); },
