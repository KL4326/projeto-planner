...
const app = {
    ...
    bindEvents() {
        document.getElementById('login-form')?.addEventListener('submit', async (e) => { e.preventDefault(); try { await signInWithEmailAndPassword(auth, document.getElementById('login-email').value, document.getElementById('login-password').value); } catch(err) { alert("Erro no acesso."); } });
    ...
    // --- LOGS & NOTIFICAÇÕES (VERSÃO ROBUSTA) ---
    async addLog(msg) {
        try {
    ...
    listenToNotifications() {
        onSnapshot(query(collection(db, "notificacoes"), orderBy("createdAt", "desc"), limit(20)), { includeMetadataChanges: true }, snap => {
            const list = document.getElementById('notif-list'); const badge = document.getElementById('notif-badge'); if(!list) return;
            list.innerHTML = ''; let unread = 0;
            snap.forEach(d => {
                const dt = d.data();
                const ts = dt.createdAt ? dt.createdAt.seconds : (dt.ts_backup / 1000);
                const time = new Date(ts * 1000).toLocaleTimeString('pt-PT', {hour:'2-digit', minute:'2-digit'});
                if (ts > this.lastReadNotif) unread++;
                list.innerHTML += `<div class="p-4 border-b dark:border-slate-800"><p class="text-xs font-medium text-slate-700 dark:text-slate-200">${dt.text}</p><div class="flex justify-between mt-2 text-[8px] font-black text-slate-400 uppercase"><span>${dt.author}</span><span>${time}</span></div></div>`;
            });
    ...
    renderDetails(id) {
        ...
        this.unsubs.push(onSnapshot(doc(db, "tarefas", id), (d) => {
            ...
            container.innerHTML = `
                ...
                <div class="bg-white dark:bg-slate-900 p-8 rounded-3xl border dark:border-slate-800 shadow-xl"><h1 class="text-4xl font-black mb-4">${t.title}</h1><p class="text-slate-500 whitespace-pre-line leading-relaxed mb-8 text-sm">${t.description || '...'}</p>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 border-t dark:border-slate-800 pt-6">
                        ...
                        <div class="flex flex-col gap-1 items-start"><span class="text-[9px] font-black uppercase text-slate-400 mb-2">Anexos</span><div id="task-att-list" class="flex flex-wrap gap-2"></div><button onclick="app.handleFileUpload('task', '${id}')" class="mt-2 text-[10px] font-black uppercase text-primary hover:opacity-70 flex items-center gap-1"><span class="material-symbols-outlined text-sm">attach_file</span> ANEXAR</button></div>
                    </div>
                </div>
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="flex flex-col gap-4"><div class="flex items-center justify-between p-2"><h2 class="font-black uppercase text-xs text-slate-400">Subtarefas</h2><button onclick="app.openSubtaskForm()" class="bg-primary text-white px-4 py-2 rounded-xl text-[10px] font-black shadow-lg">Adicionar subtarefa</button></div><div id="subtasks-list" class="bg-white dark:bg-slate-900 rounded-3xl border dark:border-slate-800 divide-y dark:divide-slate-800 overflow-hidden shadow-sm"></div></div>
                    <div class="flex flex-col gap-4"><h2 class="font-black uppercase text-xs text-slate-400 p-2">Discussão</h2><div class="bg-white dark:bg-slate-900 rounded-3xl border dark:border-slate-800 flex flex-col h-[400px] shadow-xl overflow-hidden"><div id="chat-messages" class="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar"></div><div class="p-4 border-t dark:border-slate-800 flex gap-2 bg-slate-50 dark:bg-slate-800/20"><input id="chat-input" type="text" class="flex-1 bg-white dark:bg-slate-800 border-none rounded-xl px-4 text-sm outline-none dark:text-white shadow-sm" placeholder="Escreva algo..."><button onclick="app.sendChatMessage()" class="bg-primary text-white size-10 rounded-xl flex items-center justify-center"><span class="material-symbols-outlined">send</span></button></div></div></div>
                </div>
                <div class="flex gap-4 mt-6"><button onclick="app.openEditModal()" class="flex-1 bg-yellow-500 text-white py-4 rounded-2xl font-black shadow-lg uppercase text-xs tracking-widest">Editar Tarefa</button><button id="delete-btn" class="bg-red-500 text-white px-8 py-4 rounded-2xl font-black shadow-lg uppercase text-xs tracking-widest">Excluir</button></div>
            `;
            ...
            document.getElementById('delete-btn').onclick = () => app.handleDeleteTask(id);
    ...
    async openSubtaskView(sid) {
        ...
        cont.innerHTML = `
            <div class="w-full md:w-1/2 p-8 border-r dark:border-slate-800 overflow-y-auto flex flex-col gap-6 bg-white dark:bg-slate-900">
                ...
                <div><h3 class="text-3xl font-black text-primary mb-2">${d.title}</h3><p class="text-sm text-slate-500 italic">${d.description || '...'}</p></div>
                <div class="grid grid-cols-2 gap-4 border-t dark:border-slate-800 pt-6"><div><span class="text-[9px] font-black uppercase tracking-widest">Equipa</span><p class="text-xs font-bold">${d.assignees?.join(', ') || '---'}</p></div><div><span class="text-[9px] font-black uppercase tracking-widest">Prazo</span><p class="text-xs font-bold">${dateStr}</p></div></div>
                <div class="flex flex-col border-t dark:border-slate-800 pt-4"><div id="sub-att-list" class="flex flex-wrap gap-2"></div><button onclick="app.handleFileUpload('sub', '${sid}')" class="mt-3 text-[10px] font-black uppercase text-primary flex items-center gap-1"><span class="material-symbols-outlined text-sm">attach_file</span> ANEXAR</button></div>
                <div class="flex gap-2 mt-auto pt-6"><button onclick="app.openSubtaskForm('${sid}')" class="flex-1 bg-yellow-500 text-white py-3 rounded-xl font-black text-[10px] uppercase">Editar subtarefa</button><button onclick="app.deleteSub('${sid}')" class="bg-red-500/10 text-red-500 px-4 rounded-xl hover:bg-red-500 transition-all"><span class="material-symbols-outlined text-sm">delete</span></button></div>
            </div>
        ...
        `;
        const sl = document.getElementById('sub-att-list'); (d.anexos || []).forEach(a => { sl.innerHTML += `<a href="${a.data}" download="${a.nome}" class="p-2 bg-white dark:bg-slate-800 rounded-lg border text-[9px] font-bold shadow-sm">${a.nome}</a>`; });
    ...
    // --- REUTILIZÁVEIS ---
    openEditModal() { const t = this.activeTaskData; document.getElementById('edit-task-title').value = t.title; document.getElementById('edit-task-desc').value = t.description || ""; document.getElementById('edit-task-priority').value = t.priority || "medium"; document.getElementById('edit-task-date').value = t.dueDate || ""; document.querySelectorAll('.edit-assignees-checkboxes-item').forEach(cb => cb.checked = t.assignees?.includes(cb.value)); document.getElementById('modal-backdrop').classList.replace('hidden', 'flex'); document.getElementById('modal-edit-task').classList.remove('hidden'); },
    ...
    async handleSaveSubtask() { const t = document.getElementById('sub-title-inp').value; if(!t) return; const assignees = Array.from(document.querySelectorAll('.sub-assignees-checkboxes-item:checked')).map(cb => cb.value); const data = { title: t, description: document.getElementById('sub-desc-inp').value, priority: document.getElementById('sub-priority-inp').value, dueDate: document.getElementById('sub-date-inp').value, assignees, updatedAt: serverTimestamp(), ts_backup: Date.now() }; if (this.editSubId) { await updateDoc(doc(db, "tarefas", this.currentTaskId, "subtarefas", this.editSubId), data); await this.addLog(`Editou a subtarefa "${t}"`); } else { await addDoc(collection(db, "tarefas", this.currentTaskId, "subtarefas"), { ...data, completed: false, createdAt: serverTimestamp() }); await this.addLog(`Adicionou a subtarefa "${t}"`); } this.closeModal(); },
    ...
    listenToSubChat(sid) { this.unsubs.push(onSnapshot(query(collection(db,"tarefas",this.currentTaskId,"subtarefas",sid,"comentarios"), orderBy("createdAt","asc")), s => { const c = document.getElementById('sub-chat-messages'); if(c) { c.innerHTML = ''; s.forEach(doc => { const d = doc.data(); const isMe = d.createdBy === auth.currentUser.uid; c.innerHTML += `<div class="flex flex-col ${isMe?'items-end':'items-start'}"><div class="${isMe?'bg-primary text-white rounded-br-none':'bg-white dark:bg-slate-800 rounded-bl-none'} p-3 rounded-2xl text-xs shadow-sm font-medium">${d.text}</div></div>`; }); c.scrollTop = c.scrollHeight; } })); },
