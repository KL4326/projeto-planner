...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        if (currentPage === "login.html" || currentPage === "") window.location.href = "index.html";
    ...
    } else if (currentPage !== "login.html") window.location.href = "login.html";
});
...
if (currentPage === "index.html" || currentPage === "") {
    ...
    if (tasksContainer) {
        let filterStatus = "Todas", filterSector = "Todos", filterAssignee = "Todos", filterSearch = "", data = [];
        ...
        const render = () => {
            ...
            data.forEach(snap => {
                ...
                const matchSearch = t.title.toLowerCase().includes(filterSearch.toLowerCase());
                const matchStatus = filterStatus === "Todas" || t.status === filterStatus;
                const matchSector = filterSector === "Todos" || t.sector === filterSector;
                const matchAssignee = filterAssignee === "Todos" || (t.assignees && t.assignees.includes(filterAssignee));

                if (matchSearch && matchStatus && matchSector && matchAssignee) {
                    count++;
        ...
        const renderRanking = (allDocs) => {
            ...
            const pts = {}; 
            allDocs.forEach(d => { if(d.data().status === "Concluída") (d.data().assignees || ["Equipe"]).forEach(p => pts[p] = (pts[p] || 0) + 1); });
            const sortedRank = Object.entries(pts).sort((a,b)=>b[1]-a[1]);
            rankingContainer.innerHTML = sortedRank.length ? '' : '<p class="text-center py-4 text-xs text-slate-500">Sem pontuação.</p>';
            sortedRank.forEach(([nome, pontos], i) => {
                const pos = i + 1;
                let crownSVG = "";
                if(pos <= 3) {
                    const corCoroa = pos === 1 ? "text-yellow-500" : pos === 2 ? "text-slate-400" : "text-amber-600";
                    crownSVG = `<svg class="w-4 h-4 ${corCoroa} ml-1" fill="currentColor" viewBox="0 0 24 24"><path d="M5 16L3 5L8.5 10L12 4L15.5 10L21 5L19 16H5ZM19 19C19 19.5523 18.5523 20 18 20H6C5.44772 20 5 19.5523 5 19V18H19V19Z"/></svg>`;
                }
                rankingContainer.innerHTML += `<div class="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/40 rounded-lg border border-slate-100 dark:border-slate-800 mb-1 shadow-sm"><div class="flex items-center gap-3"><span class="font-black text-slate-400 w-5 text-center text-xs">${pos}º</span><div class="flex items-center gap-1"><span class="text-sm font-bold text-slate-700 dark:text-slate-200">${nome}</span>${crownSVG}</div></div><span class="font-black text-green-600 text-[10px] bg-green-500/10 px-2 py-0.5 rounded">${pontos} pts</span></div>`;
            });
        };

        onSnapshot(query(collection(db, "tarefas"), orderBy("createdAt", "desc")), (snap) => {
            data = snap.docs; render(); renderRanking(data);
        });

        // Eventos de Busca e Filtro
        searchInput.oninput = (e) => { filterSearch = e.target.value; render(); };
        document.querySelectorAll('.filter-btn').forEach(btn => btn.onclick = () => {
            ...
            btn.classList.remove('bg-slate-100','dark:bg-slate-800','text-slate-600','dark:text-slate-300'); btn.classList.add('bg-primary','text-white'); filterStatus = btn.dataset.filter; render();
        });
        carregarUsers('filter-assignee');
        document.getElementById('filter-sector').onchange = (e) => { filterSector = e.target.value; render(); };
        document.getElementById('filter-assignee').onchange = (e) => { filterAssignee = e.target.value; render(); };
...
// --- NOVA TAREFA ---
if (currentPage === "nova-tarefa.html") {
...
// --- DETALHES ---
if (currentPage === "detalhes-tarefa.html") {
    ...
    onSnapshot(docRef, (d) => {
        ...
        tags.innerHTML = `<span class="px-2 py-0.5 rounded text-[10px] font-black uppercase text-white ${p.bg}">${p.label}</span>`;
    });

    const qsBtn = document.getElementById('quick-status-save');
    qsBtn.onclick = async () => { await updateDoc(docRef, { status: document.getElementById('quick-status-select').value }); darFeedback(qsBtn, "Salvar Status", "Atualizado!"); };

    const enviarChat = async () => {
        const i = document.getElementById('chat-input'); if(!i.value.trim()) return;
        await addDoc(collection(db, "tarefas", taskId, "comentarios"), { text: i.value, authorName: auth.currentUser.displayName || auth.currentUser.email.split('@')[0], createdBy: auth.currentUser.uid, createdAt: serverTimestamp() });
        i.value = '';
    };
    document.getElementById('send-chat-btn').onclick = enviarChat;
    document.getElementById('chat-input').onkeydown = (e) => { if(e.key === 'Enter') enviarChat(); };

    onSnapshot(query(collection(db, "tarefas", taskId, "comentarios"), orderBy("createdAt", "asc")), (snap) => {
        const ct = document.getElementById('chat-container'); ct.innerHTML = '';
    ...
    const openSub = async (id) => {
        ...
        document.getElementById('view-sub-title').innerText = d.title;
        document.getElementById('view-sub-desc').innerText = d.description || "Sem descrição.";
        ...
        unSubChat = onSnapshot(query(collection(db, "tarefas", taskId, "subtarefas", id, "comentarios"), orderBy("createdAt", "asc")), (snap) => {
            const ct = document.getElementById('sub-chat-container'); ct.innerHTML = '';
            snap.forEach(c => { const isMe = c.data().createdBy === auth.currentUser.uid; ct.innerHTML += `<div class="flex flex-col ${isMe?'items-end':'items-start'} mb-2"><span class="text-[9px] text-slate-500">${c.data().authorName}</span><div class="${isMe?'bg-primary text-white':'bg-slate-200 dark:bg-slate-800'} p-2 rounded-lg text-xs">${c.data().text}</div></div>`; });
            ct.scrollTop = ct.scrollHeight;
        });
    };

    const enviarSubChat = async () => {
        const i = document.getElementById('sub-chat-input'); if(!i.value.trim() || !activeSid) return;
        await addDoc(collection(db, "tarefas", taskId, "subtarefas", activeSid, "comentarios"), { text: i.value, authorName: auth.currentUser.displayName || auth.currentUser.email.split('@')[0], createdBy: auth.currentUser.uid, createdAt: serverTimestamp() });
        i.value = '';
    };
    document.getElementById('send-sub-chat-btn').onclick = enviarSubChat;
    document.getElementById('sub-chat-input').onkeydown = (e) => { if(e.key === 'Enter') enviarSubChat(); };

    onSnapshot(query(collection(db, "tarefas", taskId, "subtarefas"), orderBy("createdAt", "asc")), (snap) => {
        const sl = document.getElementById('subtasks-list'); sl.innerHTML = '';
        snap.forEach(sd => {
            const s = sd.data(); const dv = document.createElement('div'); dv.className = "flex items-center gap-4 px-6 py-4 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer border-b dark:border-slate-800";
            dv.innerHTML = `<input type="checkbox" ${s.completed?'checked':''} class="sc" data-id="${sd.id}"><div class="flex-1 tv" data-id="${sd.id}"><p class="font-bold ${s.completed?'line-through opacity-50':''}">${s.title}</p></div>`;
            sl.appendChild(dv);
        });
    ...
    document.getElementById('save-edit-btn').onclick = async () => {
        ...
        darFeedback(eb, "Salvar Alterações", "Guardado!");
        setTimeout(() => document.getElementById('edit-modal').classList.replace('flex', 'hidden'), 1500);
    };
    document.querySelectorAll('#close-edit-modal, #cancel-edit-btn, #close-subtask-modal, #cancel-subtask-btn, #close-view-subtask').forEach(b => b.onclick = () => { document.getElementById('edit-modal').classList.add('hidden'); document.getElementById('subtask-modal').classList.add('hidden'); document.getElementById('view-subtask-modal').classList.add('hidden'); });
...
if (currentPage === "perfil.html") {
    ...
    spBtn.onclick = async () => {
        ...
        darFeedback(spBtn, "Guardar Alterações", "Perfil Guardado!");
...
if (currentPage === "admin.html") {
    onSnapshot(collection(db, "tarefas"), (snap) => {
        ...
        snap.forEach(d => {
            const t = d.data(); if(t.status === "Concluída") done++;
            (t.assignees || ["Equipa"]).forEach(p => { if(!users[p]) users[p] = { c:0, d:0 }; users[p].c++; if(t.status === "Concluída") users[p].d++; });
        });
        document.getElementById('stat-total-tasks').innerText = total;
        document.getElementById('stat-completed-tasks').innerText = done;
        document.getElementById('stat-active-users').innerText = Object.keys(users).length;
        const ut = document.getElementById('admin-users-table');
        if(ut) { ut.innerHTML = ''; Object.entries(users).forEach(([n, s]) => { ut.innerHTML += `<tr class="border-b dark:border-slate-800"><td class="p-4 font-bold text-slate-900 dark:text-slate-100">${n}</td><td class="p-4 text-center">${s.c}</td><td class="p-4 text-center text-green-600">${s.d}</td><td class="p-4 text-center font-black">${Math.round((s.d/s.c)*100)}%</td></tr>`; }); }
    });
}
