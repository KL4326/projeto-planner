...
const app = {
    ...
    // --- LOGS (IMUNE A ERROS DE ÍNDICE) ---
    async addLog(msg) {
        await addDoc(collection(db, "notificacoes"), {
            text: msg,
            author: auth.currentUser.displayName || auth.currentUser.email,
            ts: Date.now() // Usamos timestamp numérico para evitar bloqueios de índice do servidor
        });
    },
    ...
    // --- TAREFAS MOTOR (IMUNE A ERROS DE ÍNDICE) ---
    listenToTasks() { 
    ...
    renderDashboard() {
        ...
        const sorted = this.allTasks.map(d => ({id: d.id, ...d.data()})).sort((a,b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0));
    ...
    async openSubtaskView(sid) {
        ...
        cont.innerHTML = `
            <div class="w-full md:w-1/2 p-8 border-r dark:border-slate-800 overflow-y-auto flex flex-col gap-6 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100">
                ...
                <div class="grid grid-cols-2 gap-4 border-t dark:border-slate-800 pt-6"><div><span class="text-[9px] font-black uppercase tracking-widest">Responsáveis</span><p class="text-xs font-bold">${d.assignees?.join(', ') || '---'}</p></div><div><span class="text-[9px] font-black uppercase tracking-widest">Prazo</span><p class="text-xs font-bold">${dateStr}</p></div></div>
    ...
    async removeProfilePhoto() { if(confirm("Remover?")) { this.tempPhotoBase64 = ""; const av = document.getElementById('profile-page-avatar'); av.style.backgroundImage = 'none'; av.innerText = (auth.currentUser.displayName || auth.currentUser.email).substring(0,2).toUpperCase(); document.getElementById('photo-options-perfil').classList.add('hidden'); } }
};
