...
const app = {
    ...
    bindEvents() {
        document.getElementById('login-form')?.addEventListener('submit', async (e) => {
            ...
            try { await signInWithEmailAndPassword(auth, email, pass); } catch(err) { alert("Erro no login."); }
        });
    ...
    // --- LOGS (SEM ORDERBY NO BANCO) ---
    async addLog(msg) {
        await addDoc(collection(db, "notificacoes"), {
            text: msg, author: auth.currentUser.displayName || auth.currentUser.email, ts: Date.now()
        });
    },
    listenToNotifications() {
        onSnapshot(collection(db, "notificacoes"), snap => {
            ...
            const logs = snap.docs.map(d => d.data()).sort((a,b) => b.ts - a.ts).slice(0, 10);
            list.innerHTML = logs.length ? '' : '<p class="p-8 text-center text-xs text-slate-400 italic">Vazio.</p>';
            let unread = 0;
            logs.forEach(dt => {
                if ((dt.ts / 1000) > this.lastReadNotif) unread++;
                const time = new Date(dt.ts).toLocaleTimeString('pt-PT', {hour:'2-digit', minute:'2-digit'});
                list.innerHTML += `<div class="p-4 border-b dark:border-slate-800"><p class="text-xs font-medium">${dt.text}</p><div class="flex justify-between mt-1 text-[8px] font-black uppercase text-slate-400"><span>${dt.author}</span><span>${time}</span></div></div>`;
            });
    ...
    // --- TAREFAS MOTOR (SEM ORDERBY NO BANCO) ---
    listenToTasks() { 
            ...
            container.innerHTML = `
                ...
                <div class="bg-white dark:bg-slate-900 p-8 rounded-3xl border dark:border-slate-800 shadow-xl text-left"><h1 class="text-4xl font-black mb-4">${t.title}</h1><p class="text-slate-500 whitespace-pre-line leading-relaxed mb-8 text-sm">${t.description || '...'}</p>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 border-t dark:border-slate-800 pt-6">
                        <div class="flex flex-col gap-1"><span class="text-[9px] font-black uppercase text-slate-400">Equipa</span><span class="text-xs font-bold text-primary">${t.assignees?.join(', ') || '---'}</span></div>
                        ...
                        <div class="flex flex-col gap-1 items-start flex-1"><span class="text-[9px] font-black uppercase text-slate-400 mb-2">Anexos</span><div id="task-att-list" class="flex flex-wrap gap-2"></div><button onclick="app.handleFileUpload('task', '${id}')" class="mt-2 text-[10px] font-black uppercase text-primary flex items-center gap-1"><span class="material-symbols-outlined text-sm">attach_file</span> ANEXAR</button></div>
                    </div>
                </div>
    ...
    async openSubtaskView(sid) {
        ...
        cont.innerHTML = `
            <div class="w-full md:w-1/2 p-8 border-r dark:border-slate-800 overflow-y-auto flex flex-col gap-6 bg-white dark:bg-slate-900">
                <div class="flex items-center justify-between font-black text-[10px] uppercase text-slate-400">Subtarefa<button onclick="app.closeModal()"><span class="material-symbols-outlined">close</span></button></div>
                <h3 class="text-3xl font-black text-primary">${d.title}</h3>
                <div class="grid grid-cols-2 gap-4 border-t dark:border-slate-800 pt-6"><div><span class="text-[9px] font-black uppercase tracking-widest">Equipa</span><p class="text-xs font-bold text-slate-700 dark:text-slate-300">${d.assignees?.join(', ') || '---'}</p></div><div><span class="text-[9px] font-black uppercase tracking-widest">Prazo</span><p class="text-xs font-bold text-slate-700 dark:text-slate-300">${dateStr}</p></div></div>
            ...
            <div class="flex-1 flex flex-col bg-slate-50 dark:bg-slate-900/40 text-left">
    ...
    async handleDeleteTask(id) { if(confirm("Apagar?")) { const d = await getDoc(doc(db,"tarefas",id)); await this.addLog(`Eliminou a tarefa: "${d.data().title}"`); await deleteDoc(doc(db,"tarefas",id)); this.navigate('dashboard'); } },
    async loadProfileData() { const u = auth.currentUser; if(!u) return; const d = await getDoc(doc(db, "usuarios", u.uid)); const dt = d.data() || {}; document.getElementById('profile-name-input').value = u.displayName || ""; document.getElementById('profile-bio-input').value = dt.bio || ""; document.getElementById('profile-page-name').innerText = u.displayName || "Usuário"; const av = document.getElementById('profile-page-avatar'); if(u.photoURL) av.style.backgroundImage = `url('${u.photoURL}')`; else av.innerText = (u.displayName || u.email).substring(0,2).toUpperCase(); },
