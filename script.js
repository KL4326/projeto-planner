...
const app = {
    ...
    listenToNotifications() {
        onSnapshot(collection(db, "notificacoes"), snap => {
            ...
            list.innerHTML = logs.length ? '' : '<p class="p-8 text-center text-xs text-gray-400">Sem logs.</p>';
            logs.slice(0, 15).forEach(dt => {
                const time = new Date(dt.ts).toLocaleTimeString('pt-PT', {hour:'2-digit', minute:'2-digit'});
                list.innerHTML += `<div class="p-4 border-b dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors text-left"><p class="text-xs font-bold text-gray-700 dark:text-gray-200">${dt.text}</p><div class="flex justify-between mt-1 text-[8px] font-black uppercase text-gray-400"><span>${dt.author}</span><span>${time}</span></div></div>`;
            });
        });
    },
    ...
    renderDashboard() {
        ...
        // CONTAR STATS SEMPRE DO TOTAL (Fix solicitado)
        const stats = { 'Atrasado': 0, 'Em aberto': 0, 'Em andamento': 0, 'Concluída': 0, 'Cancelada': 0 };
        const hoje = new Date().toISOString().split('T')[0];
        sorted.forEach(t => {
            let statusReal = t.status;
            if (statusReal !== 'Concluída' && statusReal !== 'Cancelada' && t.dueDate < hoje) statusReal = 'Atrasado';
            stats[statusReal]++;
        });
    ...
    renderStats(s, total) {
        ...
        const cards = [
            ...
            {label: 'Em aberto', val: s['Em aberto'] + s['Atrasado'], color: 'primary', icon: 'pending_actions'},
    ...
    renderDetails(id) {
        ...
        this.unsubs.push(onSnapshot(doc(db, "tarefas", id), (d) => {
            ...
            let actionBtn = '';
            if (t.status === 'Em aberto' || t.status === 'Atrasado') actionBtn = `<button onclick="app.updateTaskStatus('${id}', 'Em andamento')" class="bg-primary text-white px-6 py-2 rounded-xl text-xs font-black uppercase">Iniciar tarefa</button>`;
            ...
            container.innerHTML = `
                <div class="flex items-center justify-between">
                    <button onclick="app.navigate('dashboard')" class="bg-white dark:bg-gray-800 p-2 rounded-xl shadow-sm border dark:border-white/10 hover:text-primary"><span class="material-symbols-outlined">arrow_back</span></button>
                ...
                <div class="bg-white dark:bg-gray-800 p-8 rounded-3xl border border-gray-200 dark:border-white/10 shadow-xl">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-6 border-t dark:border-white/5 pt-6">
                        ...
                        <div><span class="text-[9px] font-black uppercase text-gray-400">Prazo</span><p class="text-xs font-bold dark:text-white">${dateStr}</p></div>
                        <div class="items-start flex-1"><span class="text-[9px] font-black uppercase text-gray-400 mb-2 block text-left">Anexos</span><div id="task-att-list" class="flex flex-wrap gap-2"></div><button onclick="app.handleFileUpload('task', '${id}')" class="mt-3 text-[10px] font-black uppercase text-primary flex items-center gap-1 hover:opacity-70"><span class="material-symbols-outlined text-sm">attach_file</span> ANEXAR</button></div>
                    </div>
                </div>
            ...
            `;
            const al = document.getElementById('task-att-list'); (t.anexos || []).forEach(a => { al.innerHTML += `<a href="${a.data}" download="${a.nome}" class="p-2 bg-gray-100 dark:bg-white/5 rounded-lg border dark:border-white/10 text-[9px] font-bold truncate max-w-[140px]">${a.nome}</a>`; });
    ...
    async openSubtaskView(sid) {
        ...
        cont.innerHTML = `
            <div class="w-full md:w-1/2 p-8 border-r dark:border-white/10 overflow-y-auto flex flex-col gap-6 bg-white dark:bg-gray-800 text-left">
                <div class="flex items-center justify-between font-black text-[10px] uppercase text-gray-400">Detalhes da Subtarefa<button onclick="app.closeModal()"><span class="material-symbols-outlined">close</span></button></div>
                <div><h3 class="text-3xl font-black text-primary mb-2">${d.title}</h3><p class="text-sm text-gray-500 dark:text-gray-300 italic leading-relaxed border-l-4 border-primary/20 pl-4 py-1">${d.description || 'Sem descrição.'}</p></div>
                <div class="grid grid-cols-2 gap-4 border-t dark:border-white/5 pt-6"><div><span class="text-[9px] font-black uppercase text-gray-400">Responsáveis</span><p class="text-xs font-bold dark:text-white">${d.assignees?.join(', ') || '---'}</p></div><div><span class="text-[9px] font-black uppercase text-gray-400">Prazo</span><p class="text-xs font-bold dark:text-white">${d.dueDate ? new Date(d.dueDate).toLocaleDateString('pt-PT') : '---'}</p></div></div>
    ...
    loadUsers() { onSnapshot(collection(db, "usuarios"), (snap) => { const opts = snap.docs.map(d => d.data().nome); ['task-assignees-checkboxes', 'edit-assignees-checkboxes', 'sub-assignees-checkboxes'].forEach(cid => { const el = document.getElementById(cid); if (el) el.innerHTML = opts.map(n => `<label class="flex items-center gap-2 p-2 hover:bg-gray-100 dark:hover:bg-white/5 rounded cursor-pointer transition-all"><input type="checkbox" value="${n}" class="${cid}-item rounded text-primary w-4 h-4"><span class="text-xs font-bold text-gray-700 dark:text-slate-300">${n}</span></label>`).join(''); }); }); },
    ...
    async handleSaveProfile() { try { await updateProfile(auth.currentUser, { displayName: document.getElementById('profile-name-input').value }); await setDoc(doc(db,"usuarios",auth.currentUser.uid), { nome: document.getElementById('profile-name-input').value, bio: document.getElementById('profile-bio-input').value, foto: this.tempPhotoBase64 || auth.currentUser.photoURL }, {merge:true}); this.showToast("Perfil salvo!"); this.navigate('dashboard'); } catch(e) { alert("Erro."); } },
