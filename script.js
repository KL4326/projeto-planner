...
const app = {
    ...
    bindEvents() {
        ...
        document.getElementById('login-form')?.addEventListener('submit', async (e) => {
            ...
            } catch(err) { alert("E-mail ou senha incorretos."); btn.innerText = "Entrar"; }
        });
        ...
        const resetFn = async () => { if(auth.currentUser) { await sendPasswordResetEmail(auth, auth.currentUser.email); alert("E-mail de redefinição enviado!"); } };
    ...
    checkAuth() {
        onAuthStateChanged(auth, (user) => {
            if (user) {
                ...
                if(user.email === "SEU_EMAIL_AQUI@gmail.com") document.getElementById('admin-menu-link').classList.replace('hidden', 'flex');
            } else {
    ...
    renderDashboard() {
        ...
        this.allTasks.forEach(docSnap => {
            ...
            if(mSearch && mStat && mSect && mAssign) {
                ...
                const date = t.dueDate ? new Date(t.dueDate).toLocaleDateString('pt-PT') : '---';
                ...
                div.innerHTML = `<div class="flex items-center gap-4 flex-1"><button onclick="app.quickComplete('${docSnap.id}')" class="p-2 rounded-lg ${t.status==='Concluída'?'text-green-500 bg-green-500/10':'text-slate-400 bg-slate-100 dark:bg-slate-700'}"><span class="material-symbols-outlined">${icon}</span></button><div class="flex flex-col"><span class="font-bold text-slate-900 dark:text-white">${t.title}</span><div class="flex items-center gap-3 mt-1 text-[10px] font-bold uppercase"><span class="text-primary font-black">${t.sector || '---'}</span><span class="text-slate-400">| ${t.assignees?.join(', ') || '---'}</span><span class="text-slate-400">| ${date}</span></div></div></div><span class="px-3 py-1 ${p.bg} text-white text-[10px] font-black rounded-full uppercase">${p.label}</span>`;
    ...
    async handleSaveProfile() {
        ...
        this.darVerde(btn, "Guardar Alterações", "Guardado!");
        setTimeout(() => this.navigate('dashboard'), 1000);
    },
    ...
    renderDetails(id) {
        ...
        onSnapshot(doc(db, "tarefas", id), (d) => {
            ...
            const t = d.data();
            const p = CONFIG.prioridades[t.priority] || CONFIG.prioridades.low;
            cont.innerHTML = `<div class="p-8 bg-white dark:bg-slate-900 rounded-2xl border dark:border-slate-800 shadow-xl mb-8"><div class="flex items-center gap-2 mb-4"><span class="px-2 py-0.5 rounded text-[10px] font-black uppercase text-white ${p.bg}">${p.label}</span></div><h1 class="text-3xl font-black mb-2">${t.title}</h1><p class="text-slate-500 whitespace-pre-line mb-8 leading-relaxed">${t.description || 'Sem descrição.'}</p><div class="flex flex-wrap gap-6 text-[11px] font-black uppercase text-slate-400 border-t dark:border-slate-800 pt-6"><div class="flex items-center gap-1 text-primary"><span class="material-symbols-outlined text-sm">group</span> ${t.assignees?.join(', ') || '---'}</div><div class="flex items-center gap-1"><span class="material-symbols-outlined text-sm">domain</span> ${t.sector || '---'}</div><div class="flex items-center gap-1"><span class="material-symbols-outlined text-sm">calendar_today</span> ${t.dueDate || '---'}</div></div><div class="mt-8 flex gap-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl w-fit border dark:border-slate-700"><select id="quick-status-sel" class="bg-transparent border-none text-sm font-bold text-primary outline-none"><option value="Em aberto" ${t.status==='Em aberto'?'selected':''}>Em aberto</option><option value="Em andamento" ${t.status==='Em andamento'?'selected':''}>Em andamento</option><option value="Concluída" ${t.status==='Concluída'?'selected':''}>Concluída</option><option value="Cancelada" ${t.status==='Cancelada'?'selected':''}>Cancelada</option></select><button id="quick-status-btn" class="bg-primary text-white text-[10px] font-black px-4 py-2 rounded-lg">Salvar Status</button><button id="edit-task-btn" class="bg-slate-200 dark:bg-slate-700 px-4 py-2 rounded-lg text-[10px] font-black">Editar</button><button id="delete-task-btn" class="bg-red-500/10 text-red-500 px-4 py-2 rounded-lg text-[10px] font-black">Excluir</button></div></div><div class="flex flex-col gap-4 mb-8"><div class="flex items-center justify-between"><h2 class="text-xl font-black">Subtarefas</h2><span id="sub-prog" class="text-xs font-bold bg-primary/10 text-primary px-2 py-1 rounded-full">0/0</span></div><div id="subtasks-list" class="bg-white dark:bg-slate-900 rounded-xl border dark:border-slate-800 divide-y dark:divide-slate-800 overflow-hidden"></div><button onclick="app.openSubtaskForm()" class="py-3 bg-primary text-white rounded-xl font-bold">Nova Subtarefa</button></div><div class="bg-white dark:bg-slate-900 rounded-2xl border dark:border-slate-800 flex flex-col h-[450px] shadow-xl overflow-hidden"><div id="main-chat" class="flex-1 overflow-y-auto p-6 space-y-4"></div><div class="p-4 border-t dark:border-slate-800 flex gap-2"><input id="main-chat-inp" class="flex-1 bg-slate-50 dark:bg-slate-800 rounded-xl px-4 text-sm outline-none dark:text-white" placeholder="Mensagem..."><button id="main-chat-send" class="bg-primary text-white size-10 rounded-lg flex items-center justify-center"><span class="material-symbols-outlined">send</span></button></div></div>`;
            
            document.getElementById('quick-status-btn').onclick = (e) => { updateDoc(doc(db,"tarefas",id), {status: document.getElementById('quick-status-sel').value}); app.darVerde(e.target, "Salvar Status", "Ok!"); };
            document.getElementById('edit-task-btn').onclick = () => app.openEditTaskModal(t);
            document.getElementById('delete-task-btn').onclick = async () => { if(confirm("Apagar?")) { await deleteDoc(doc(db,"tarefas",id)); app.navigate('dashboard'); } };
    ...
    listenToSubtasks(tid) {
        onSnapshot(query(collection(db,"tarefas",tid,"subtarefas"), orderBy("createdAt","asc")), (snap) => {
            ...
            snap.forEach(sd => {
                const s = sd.data();
                const div = document.createElement('div');
                div.className = "flex items-center gap-4 px-6 py-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer";
                div.onclick = (e) => { if(e.target.type !== 'checkbox') this.openSubtaskView(sd.id); };
                div.innerHTML = `<input type="checkbox" ${s.completed?'checked':''} onchange="app.toggleSub('${sd.id}', this.checked)" class="rounded text-primary"><div class="flex-1 font-bold ${s.completed?'line-through opacity-40':''} text-sm">${s.title}</div>`;
    ...
    async openSubtaskView(sid) {
        ...
        content.innerHTML = `<div class="w-full md:w-1/3 p-6 border-r dark:border-slate-800 overflow-y-auto flex flex-col gap-6"><div><div class="flex items-center justify-between mb-4"><span class="text-[10px] font-black uppercase text-slate-400">Info</span><button onclick="app.closeModal()"><span class="material-symbols-outlined text-slate-400">close</span></button></div><h3 class="text-xl font-black text-primary mb-2">${d.title}</h3><p class="text-sm text-slate-500 italic">${d.description || '...'}</p></div><div><label class="text-[10px] font-black uppercase text-slate-400">Responsáveis</label><p class="text-sm font-bold">${d.assignees?.join(', ') || '---'}</p></div><div><label class="text-[10px] font-black uppercase text-slate-400">Prazo</label><p class="text-sm font-bold">${d.dueDate || '---'}</p></div><div class="flex gap-2 mt-4"><button onclick="app.openSubtaskForm('${sid}')" class="flex-1 py-2 bg-slate-100 dark:bg-slate-800 rounded-lg text-xs font-bold">Editar</button><button onclick="app.deleteSub('${sid}')" class="py-2 px-3 bg-red-500/10 text-red-500 rounded-lg"><span class="material-symbols-outlined text-sm">delete</span></button></div></div><div class="flex-1 flex flex-col bg-slate-50 dark:bg-slate-900/40"><div id="sub-chat-cont" class="flex-1 overflow-y-auto p-6 space-y-3"></div><div class="p-4 border-t dark:border-slate-800 flex gap-2"><input id="sub-chat-inp" class="flex-1 bg-white dark:bg-slate-800 border-none rounded-xl px-4 text-sm outline-none dark:text-white" placeholder="Chat..."><button id="sub-chat-send" class="bg-primary text-white size-10 rounded-lg flex items-center justify-center"><span class="material-symbols-outlined">send</span></button></div></div>`;
    ...
    // --- APOIO ---
    darVerde(btn, original, sucesso) { if(!btn) return; btn.innerText = sucesso; btn.classList.replace('bg-primary', 'bg-green-500'); setTimeout(() => { btn.innerText = original; btn.classList.replace('bg-green-500', 'bg-primary'); }, 2000); },
    closeModal() { document.getElementById('modal-backdrop').classList.replace('flex', 'hidden'); document.querySelectorAll('.modal-box').forEach(m => m.classList.add('hidden')); },
    ...
    openSubtaskForm(sid = null) { this.editSubId = sid; document.getElementById('modal-subtask-form').classList.remove('hidden'); document.getElementById('modal-backdrop').classList.replace('hidden', 'flex'); },
