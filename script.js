...
const app = {
    ...
    listenToNotifications() {
        onSnapshot(collection(db, "notificacoes"), snap => {
            ...
            let unread = 0;
            logs.forEach(dt => {
                if (dt.ts > (this.lastReadNotif * 1000)) unread++;
    ...
    renderDetails(id) {
        ...
        this.unsubs.push(onSnapshot(doc(db, "tarefas", id), (d) => {
            ...
            container.innerHTML = `
                ...
                <div class="bg-white dark:bg-slate-900 p-8 rounded-3xl border dark:border-slate-800 shadow-xl text-left"><h1 class="text-4xl font-black mb-4">${t.title}</h1><p class="text-slate-500 dark:text-slate-400 whitespace-pre-line leading-relaxed mb-8 text-sm">${t.description || '...'}</p>
