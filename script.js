...
const app = {
    ...
    async handleSaveProfile() { 
        try { 
            ...
            const novaFoto = this.tempPhotoBase64;
            const updateObj = { nome: document.getElementById('profile-name-input').value, cargo: document.getElementById('profile-role-input').value, bio: document.getElementById('profile-bio-input').value };
            ...
            if (novaFoto) { avH.style.backgroundImage = `url('${novaFoto}')`; avH.innerText = ''; } 
            else if (novaFoto === "") { avH.style.backgroundImage = 'none'; avH.innerText = auth.currentUser.displayName.substring(0,2).toUpperCase(); }

            this.showToast("Perfil corporativo atualizado!"); this.navigate('dashboard'); 
        } catch(e) { this.showToast("Erro ao salvar", "error"); } 
    },
