...
const app = {
    ...
    bindEvents() {
        document.getElementById('login-form')?.addEventListener('submit', async (e) => {
            ...
            catch(err) { alert("Dados incorretos."); btn.innerText = "Entrar"; }
        });
    ...
    listenToNotifications() {
        // Removido o orderBy temporariamente para evitar erro de índice se ainda não existir
        onSnapshot(query(collection(db, "notificacoes"), limit(15)), snap => {
            const list = document.getElementById('notif-list'); const badge = document.getElementById('notif-badge'); if(!list) return;
            const logs = snap.docs.map(d => ({id: d.id, ...d.data()})).sort((a,b) => (b.ts || 0) - (a.ts || 0));
            ...
            logs.forEach(dt => {
                ...
                const time = dt.createdAt ? new Date(dt.createdAt.seconds * 1000).toLocaleTimeString('pt-PT', {hour:'2-digit', minute:'2-digit'}) : 'Agora';
                list.innerHTML += `<div class="p-4 border-b dark:border-slate-800"><p class="text-xs font-medium">${dt.text}</p><div class="flex justify-between mt-2 text-[8px] font-black text-slate-400 uppercase"><span>${dt.author}</span><span>${time}</span></div></div>`;
            });
    ...
    async handleCreateTask() {
        ...
        await addDoc(collection(db, "tarefas"), { title, description: document.getElementById('task-desc').value, priority: document.getElementById('task-priority-droplist').value, assignees, status: "Em aberto", createdAt: serverTimestamp(), createdBy: auth.currentUser.uid, dueDate: document.getElementById('task-date').value });
